...
[![Platform](https://img.shields.io/badge/平台-macOS%20%7C%20iPadOS-blue?logo=apple)](https://developer.apple.com)
...
- **macOS**：使用 Xcode 16+ 直接開啟 `Echo-Imprint.swiftpm`
- **iPadOS**：使用 Swift Playgrounds 4+ 在 iPad 上開啟 `.swiftpm` 套件
- 需要麥克風權限（首次啟動時彈出授權請求）
...
> 💡 不慎拒絕？Mac 端：**系統設定 → 隱私權與安全性 → 麥克風**；iPad 端：**設定 → 隱私權與安全性 → 麥克風**。
...
> 💡 Mac 端：**系統設定 → 輔助使用 → VoiceOver**；iPad 端：**設定 → 輔助使用 → VoiceOver**，或三次點按側邊按鈕快速切換。
