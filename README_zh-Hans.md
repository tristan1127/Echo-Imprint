...
- **macOS**：使用 Xcode 16+ 直接打开 `Echo-Imprint.swiftpm`
- **iPadOS**：使用 Swift Playgrounds 4+ 在 iPad 上打开 `.swiftpm` 包
- 需要麦克风权限（首次启动时弹出授权请求）
...
> 💡 不慎拒绝？Mac 端：**系统设置 → 隐私与安全性 → 麦克风**；iPad 端：**设置 → 隐私与安全性 → 麦克风**。
...
> 💡 Mac 端：**系统设置 → 辅助功能 → VoiceOver**；iPad 端：**设置 → 辅助功能 → VoiceOver**，或三击侧边按钮快速切换。
